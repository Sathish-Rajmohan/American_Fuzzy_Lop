...
def parse_config(config_file, overwrite_output=True):
    """Parse configuration file and set up directories"""
    try:
    ...
    initial_seed = os.listdir(conf_dict['seeds_folder'])[0]
    shutil.copyfile(os.path.join(conf_dict['seeds_folder'], initial_seed),
                    conf_dict['current_input'])

    conf_dict['raw_target_args'] = conf_dict['target_args'].copy()
